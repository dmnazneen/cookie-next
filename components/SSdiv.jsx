    ...
    return (
        <div>
            ...
            <div className='flex gap-2 mb-4'>
                {
                    item.map(m => <Image key={m.id} src={`https://localhost:9998/api/${m.image}`} alt={'screenshot of the user desktop'} width={200} height={150} unoptimized={true} priority={false} className='rounded-sm' />)
                }
            </div>
        </div>
    );
